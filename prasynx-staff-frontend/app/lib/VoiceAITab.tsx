...
const quickActions = [
  ...
  { label: 'Student Info', icon: GraduationCap, intent: 'info', color: 'text-indigo-500' },
...
export default function VoiceAITab() {
  const [messages, setMessages] = useState<Message[]>([{
    role: 'assistant',
    content: "Hello! I'm Prasunet AI Voice Assistant. I can help you with complaints, meetings, support tickets, student information, part-time jobs, and notifications. How can I assist you today?",
    timestamp: new Date(),
  }]);
  ...
  const startCall = useCallback(async () => {
    try {
      const res = await fetch(`${VOICE_API}/incoming-call`, {
        ...
        body: JSON.stringify({ from: 'staff-portal', callerName: 'Staff', callerRole: 'staff' }),
      });
  ...
  const handleQuickAction = async (intent: string) => {
    const labels: Record<string, string> = {
      complaint: 'register a complaint', appointment: 'schedule a meeting',
      ticket: 'create a support ticket', info: 'get student information',
      job: 'find a part-time job', notification: 'send a notification',
    };
    ...
    <div className="flex flex-col h-[calc(100vh-12rem)]">
      <div className="flex items-center gap-2 p-3 border-b border-gray-200 bg-white">
        ...
        <div className="flex-1">
          ...
          <div className="text-[10px] text-gray-400">Staff Portal</div>
        </div>
          ...
          <div className="flex-1 overflow-y-auto p-4 space-y-3">
            {messages.map((msg, i) => (
              <motion.div key={i} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.2 }}
                ...
                <div className={`max-w-[85%] sm:max-w-[75%] px-3 py-2 rounded-xl text-sm ${msg.role === 'user' ? 'bg-primary text-white rounded-tr-sm' : 'bg-gray-100 text-gray-800 rounded-tl-sm'}`}>{msg.content}</div>
