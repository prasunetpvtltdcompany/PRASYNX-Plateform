...
export class MarksService {
  ...
  async getGradeSummary(orgId: string) {
    const { data, error } = await supabase.from('exam_results')
      .select('marks_obtained, max_marks, student:students(full_name), exam:exams(name, class:classes(name))')
  ...
  async getReportCard(orgId: string, studentId: string, examId: string) {
    ...
    const { data: student } = await supabase.from('students')
      .select('full_name, roll_number, class_student_map!inner(class:classes(name))')
