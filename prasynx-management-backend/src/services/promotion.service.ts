...
export class PromotionService {
  async getHistory(orgId: string) {
    const { data, error } = await supabase.from('promotion_history')
      .select('*, student:students(full_name, roll_number), from_class:classes!from_class_id(name), to_class:classes!to_class_id(name), academic_year_info:academic_years(name)')
