...
export class AcademicService {
  ...
  async getSections(orgId: string, pagination?: PaginationParams) {
    let query = supabase.from('sections').select('*, class:classes(name)', { count: 'exact' }).eq('organisation_id', orgId).order('name');
    if (pagination) query = query.range(pagination.offset, pagination.offset + pagination.limit - 1);
    const { data, error, count } = await query;
    if (error) throw new BadRequestError(error.message);
    return { data: data || [], total: count };
  ...
  async getSectionById(id: string) {
    const { data, error } = await supabase.from('sections').select('*, class:classes(name)').eq('id', id).single();
    if (error) throw new NotFoundError('Section not found');
    return data;
  }

  async createSection(orgId: string, body: any) {
    const { data, error } = await supabase.from('sections').insert({ ...body, organisation_id: orgId }).select().single();
    if (error) throw new BadRequestError(error.message);
    return data;
  }
  ...
  async getClassSubjects(orgId: string, pagination?: PaginationParams) {
    let query = supabase.from('class_subjects').select('*, class:classes(name), subject:subjects(name)', { count: 'exact' }).eq('organisation_id', orgId);
    if (pagination) query = query.range(pagination.offset, pagination.offset + pagination.limit - 1);
    const { data, error, count } = await query;
    if (error) throw new BadRequestError(error.message);
    return { data: data || [], total: count };
  ...
  async getClassSubjectById(id: string) {
    const { data, error } = await supabase.from('class_subjects').select('*, class:classes(name), subject:subjects(name)').eq('id', id).single();
  ...
  async getEnrollments(orgId: string) {
    const { data, error } = await supabase.from('class_student_map')
      .select('*, class:classes(name), student:students(full_name, roll_number)')
