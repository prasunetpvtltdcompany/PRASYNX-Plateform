...
export class TimetableService {
  ...
  async detectConflicts(orgId: string, data: { teacher_id?: string; day_of_week: number; start_time: string; end_time: string; room?: string; exclude_id?: string }) {
    ...
    if (data.room) {
      let q = supabase.from('timetable_entries')
        .select('id, class_id, start_time, end_time, class:classes(name, section)')
        .eq('organisation_id', orgId)
        .eq('room', data.room)
        .eq('day_of_week', data.day_of_week)
        .eq('is_active', true)
  ...
  async getConflicts(orgId: string) {
    const { data } = await supabase
      ...
      .select('*, entry_a:entry_id_a(id, class:classes(name, section), subject:subjects(name)), entry_b:entry_id_b(id, class:classes(name, section), subject:subjects(name))')
      .eq('organisation_id', orgId)
  ...
  async getClassesList(orgId: string) {
    const { data } = await supabase.from('classes').select('id, name, section, grade_level').eq('organisation_id', orgId).eq('status', 'active');
    return data || [];
  }
