...
const STUDENT_SELECT = 'id, full_name, roll_number, blood_group, student_class:classes(name), section';
...
export class HealthService {
  ...
  async studentProfile(orgId: string, studentId: string) {
    const [studentRes, recordsRes, vaccinationsRes, medicalRes, emergencyRes] = await Promise.all([
      supabase.from('students').select('id, full_name, roll_number, blood_group, date_of_birth, gender, status, student_class:classes(name), section').eq('organisation_id', orgId).eq('id', studentId).single(),
