...
export class AssignmentService {
  ...
  async publishAssignment(assignmentId: string) {
    ...
    }).eq('id', assignmentId).select('*, class:classes(*)').single();
    if (error) throw error;
  ...
  async duplicateAssignment(assignmentId: string) {
    ...
    const { data, error: insError } = await supabase.from('assignments').insert({
      ...
      max_score: orig.max_score, instructions: orig.instructions, section: orig.section,
  ...
  private async getRecentAssignments(orgId: string, limit: number) {
    const { data } = await supabase.from('assignments').select('title, status, due_date, subject:subjects(name), class:classes(name)').eq('organisation_id', orgId).order('created_at', { ascending: false }).limit(limit);
    return data || [];
  }

  private async getUpcomingDeadlines(orgId: string, limit: number) {
    const today = new Date().toISOString().split('T')[0];
    const { data } = await supabase.from('assignments').select('title, due_date, status, subject:subjects(name), class:classes(name)').eq('organisation_id', orgId).eq('status', 'active').gte('due_date', today).order('due_date', { ascending: true }).limit(limit);
  ...
  private async getClassPerformance(orgId: string) {
    const { data } = await supabase.from('assignments').select('class_id, class:classes(name), avg_score, submitted_count, total_students').eq('organisation_id', orgId).not('avg_score', 'is', null);
    const grouped: Record<string, { name: string; totalScore: number; count: number; totalSubmitted: number; totalStudents: number }> = {};
    (data || []).forEach((a: any) => {
  ...
  private async getLowSubmissionClasses(orgId: string) {
    const { data } = await supabase.from('assignments').select('class_id, class:classes(name), total_students, submitted_count').eq('organisation_id', orgId);
  ...
  private async getClassPerformanceReport(orgId: string) {
    const { data } = await supabase.from('assignments').select('class_id, class:classes(name), total_students, submitted_count, graded_count, avg_score').eq('organisation_id', orgId);
    const grouped: Record<string, any> = {};
    (data || []).forEach((a: any) => {
      const cid = a.class_id;
      if (!cid) return;
