...
import {
  ...
  Eye, EyeOff, ArrowUpRight, Plus, Filter, MoreHorizontal, Edit3,
  HelpCircle, MessagesSquare, TicketCheck, Briefcase, Mic, Award, CalendarDays,
  ChevronLeft, ArrowRight, Loader2, Sparkles, Home, UserCheck, BarChart3,
...
export default function ParentsPage() {
  ...
  const login = async () => {
    ...
    try {
      ...
      if (res.success && res.data) {
        ...
        if (!user || user.role !== 'parent') { setError('Invalid portal. Please use the correct login portal.'); setLoading(false); return; }
        try {
          const supabase = createClient();
        ...
        if (students && students.length > 0) setSelectedChild(students[0]);
      } else setError(res.error || 'Login failed');
    } catch (error: any) { setError(error.message || 'Login failed');
  ...
  // Show login modal automatically when session is missing
  useEffect(() => {
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-sm">
      <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }}
        ...
        <div className="pointer-events-none absolute inset-0">
          <div className="absolute -top-40 -left-40 h-[500px] w-[500px] rounded-full bg-green-500/5 blur-[120px]" />
          <div className="absolute -bottom-40 -right-40 h-[500px] w-[500px] rounded-full bg-purple-500/5 blur-[120px]" />
        </div>
        <div className="relative p-6 sm:p-8">
          <div className="text-center mb-6">
            <div className="mb-3 inline-flex items-center gap-2 rounded-full border border-[#DCFCE7] bg-green-50 px-3 py-1">
              ...
              <span className="text-[10px] font-bold text-green-600">Parents Portal</span>
            </div>
            <h2 className="text-xl font-black text-[#0F172A]">Welcome Back!</h2>
            <p className="mt-1 text-sm text-[#64748B]">Sign in to track your child&apos;s progress</p>
          </div>
          {error && (
          ...
          <form onSubmit={(e) => { e.preventDefault(); login(); }} className="space-y-3.5">
            ...
            <div>
              ...
              <div className="relative">
                ...
                <button type="button" onClick={() => setShowPassword(!showPassword)} className="absolute right-3 top-1/2 -translate-y-1/2 text-[#94A3B8] transition hover:text-[#475569]">
            ...
            <div className="flex items-center justify-between">
              <label className="flex cursor-pointer items-center gap-2">
                <input type="checkbox" checked={rememberMe} onChange={() => setRememberMe(!rememberMe)} className="h-4 w-4 rounded border-[#CBD5E1] text-green-600 focus:ring-green-500" />
                <span className="text-xs font-semibold text-[#64748B]">Remember me</span>
              </label>
              <Link href="http://localhost:3000/forgot-password" className="text-xs font-bold text-green-600 transition hover:text-green-700">Forgot Password?</Link>
            </div>
            <motion.button whileHover={{ scale: 1.01 }} whileTap={{ scale: 0.98 }} type="submit" disabled={loading}
              className="flex w-full items-center justify-center gap-2 rounded-xl bg-gradient-to-r from-green-600 to-green-500 py-2.5 text-sm font-bold text-white shadow-lg shadow-green-600/20 transition-all hover:-translate-y-0.5 hover:shadow-xl disabled:cursor-not-allowed disabled:opacity-60">
              {loading ? <Loader2 size={16} className="animate-spin" /> : <><ArrowRight size={16} /> Sign In</>}
            </motion.button>
          </form>
          <div className="mt-5 space-y-3">
            <div className="text-center">
              <p className="text-xs text-[#94A3B8]">Having trouble signing in?{' '}
                <Link href="http://localhost:3000/contact" className="font-bold text-green-600 transition hover:text-green-700">Contact Support</Link>
              </p>
            </div>
    ...
    <div className="flex h-dvh overflow-hidden bg-[#F8FAFC]">
      ...
      <aside className={`dynamic-sidebar ${sidebarOpen ? 'open' : ''} ${hasMultipleChildren ? 'shifted' : ''}`}>
        ...
        <div className="dynamic-sidebar-footer">
          <button className="dynamic-sidebar-footer-item" onClick={() => { if (session) logout(); else setShowLoginModal(true); }}>
      ...
      <div className={`content-area ${hasMultipleChildren ? 'has-child-rail' : ''}`}>
        ...
        <main className="flex-1 overflow-y-auto p-4 lg:p-6">
          ...
          {message && (
            <motion.div initial={{ opacity: 0, y: -8 }} animate={{ opacity: 1, y: 0 }}
              className={`p-3.5 rounded-xl text-sm font-medium mb-6 flex items-center gap-2.5 ${messageType === 'success' ? 'bg-green-50 text-green-700 border border-green-200' : 'bg-red-50 text-red-700 border border-red-200'}`}>
              {messageType === 'success' ? <CheckCircle2 size={18} /> : <AlertCircle size={18} />} {message}
            </motion.div>
          )}
