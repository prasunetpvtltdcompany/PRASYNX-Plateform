...
import {
  ...
  CalendarDays, TicketCheck, Briefcase, Phone, Volume2,
  Loader2, FileText, Bell, PhoneOff, Speaker,
} from 'lucide-react';
...
const quickActions = [
  ...
  { label: 'Schedule Meeting', icon: CalendarDays, intent: 'appointment', color: 'text-blue-500' },
  { label: 'Create Support Ticket', icon: TicketCheck, intent: 'ticket', color: 'text-purple-500' },
  { label: 'Post Job', icon: Briefcase, intent: 'job', color: 'text-green-500' },
  { label: 'Send Notification', icon: Bell, intent: 'notification', color: 'text-orange-500' },
];
...
export default function VoiceAITab() {
  const [messages, setMessages] = useState<Message[]>([{
    role: 'assistant',
    content: "Hello! I'm Prasunet AI Voice Assistant. I can help you with complaints, appointments, support tickets, job postings, and notifications. How can I assist you today?",
    timestamp: new Date(),
  }]);
  ...
  const startCall = useCallback(async () => {
    try {
      const res = await fetch(`${VOICE_API}/incoming-call`, {
        ...
        body: JSON.stringify({ from: 'admin-portal', callerName: 'Admin', callerRole: 'admin' }),
      });
      const data = await res.json();
      if (data.success && data.data?.call?.id) {
        setCallId(data.data.call.id);
        setCallActive(true);
      }
    } catch {}
  }, []);
  ...
  useEffect(() => {
    ...
    if (messages.length === 1 && messages[0].role === 'assistant') {
      setTimeout(() => {
        speakText(messages[0].content, () => setIsSpeakingState(false));
        setIsSpeakingState(true);
      }, 500);
    }
  }, []);
  ...
  const handleSend = async (text?: string) => {
    ...
    try {
      ...
      if (!currentCallId) {
        ...
        if (callData.success && callData.data?.call?.id) {
          currentCallId = callData.data.call.id;
  ...
  const endCall = async () => {
    stopSpeaking();
    if (callId) {
      await fetch(`${VOICE_API}/calls/${callId}/end`, { method: 'POST' }).catch(() => {});
    }
    setCallActive(false);
    setCallDuration(0);
    setMessages([{
      role: 'assistant',
      content: 'Call ended. Thank you for using Prasunet AI Voice Assistant!',
      timestamp: new Date(),
    }]);
  };
    ...
    <div className="flex-1 flex flex-col">
        ...
        {isProcessing && (
          <div className="flex gap-2">
            ...
            <div className="bg-gray-100 px-3 py-2 rounded-xl rounded-tl-sm flex items-center gap-2">
              ...
              {isSpeaking && <span className="text-xs text-gray-400">Speaking...</span>}
            </div>
          </div>
        )}
        ...
        <div className="flex flex-wrap gap-1.5 mb-2">
          {quickActions.map(action => (
            <button
              ...
              disabled={isProcessing || !callActive}
      ...
      <div className="p-3 border-t border-gray-200 bg-white">
        <div className="flex items-center gap-2">
          ...
          <button
            onClick={() => handleSend()}
            disabled={!input.trim() || isProcessing || !callActive}
  ...
  const renderHistory = () => (
    <div className="flex-1 p-4">
      <div className="text-center text-gray-400 py-12">
        ...
        <p className="text-xs mt-1">Your call transcripts and summaries will appear here.</p>
      </div>
    </div>
  );
    ...
    <div className="flex flex-col h-[calc(100vh-12rem)]">
      <div className="flex items-center gap-2 p-3 border-b border-gray-200 bg-white">
        ...
        <div className="flex-1">
          ...
          <div className="text-[10px] text-gray-400">Prasunet Ecosystem</div>
        </div>
        ...
        {[
          ...
          <button
            ...
            className={`flex items-center gap-1.5 px-3 py-2 text-xs font-medium border-b-2 transition-colors ${
              activeView === view.key ? 'border-primary text-primary' : 'border-transparent text-gray-500 hover:text-gray-700'
            }`}
          >
      ...
      {activeView === 'chat' ? renderChat() : renderHistory()}
    </div>
  );
}
