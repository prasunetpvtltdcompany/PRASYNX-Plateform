...
router.post('/forgot-password', async (req: Request, res: Response) => {
  ...
  try {
    ...
    const resetLink = `${process.env.FRONTEND_URL || 'http://localhost:3006'}/reset-password?token=${token}&email=${encodeURIComponent(email)}`;
...
router.get('/dashboard', authenticate, async (req: Request, res: Response) => {
  ...
  try {
    ...
    const { data: jobs } = await supabase.from('part_time_jobs').select('id, title, status, created_at').eq('provider_id', providerId).order('created_at', { ascending: false });
    const { data: applications } = await supabase.from('part_time_job_applications').select('id, status, part_time_jobs!inner(provider_id)').eq('part_time_jobs.provider_id', providerId);
    ...
    res.json({ totalJobs, activeJobs, totalApplications: totalApps, pendingApplications: pendingApps, recentJobs: jobs?.slice(0, 5) || [] });
  } catch (error: any) {
    res.status(500).json({ error: error.message });
  }
});
...
router.get('/jobs', authenticate, async (req: Request, res: Response) => {
  ...
  try {
    ...
    if (!provider) return res.status(404).json({ error: 'Provider profile not found' });
    const providerId = provider.id;
    const { data, error } = await supabase.from('part_time_jobs').select('*').eq('provider_id', providerId).order('created_at', { ascending: false });
    if (error) throw error;
    res.json(data || []);
  } catch (error: any) {
    res.status(500).json({ error: error.message });
...
router.patch('/applications/:id/status', authenticate, async (req: Request, res: Response) => {
  ...
  try {
    ...
    if (!provider) return res.status(404).json({ error: 'Provider profile not found' });
    const providerId = provider.id;
    const { data: app } = await supabase.from('part_time_job_applications').select('id, job_id').eq('id', id).maybeSingle();
...
router.get('/dashboard/enhanced', authenticate, async (req: Request, res: Response) => {
  ...
  try {
    ...
    if (!provider) return res.status(404).json({ error: 'Provider profile not found' });
    const providerId = provider.id;
    const { data: jobs } = await supabase.from('part_time_jobs').select('id, title, status, created_at, pay_amount, pay_type').eq('provider_id', providerId).order('created_at', { ascending: false });
...
router.post('/messages', authenticate, async (req: Request, res: Response) => {
  ...
  try {
    const { data: provider } = await supabase.from('job_providers').select('id').eq('user_id', userId).maybeSingle();
    if (!provider) return res.status(404).json({ error: 'Provider profile not found' });
    const { data, error } = await supabase.from('provider_messages').insert({
      provider_id: provider.id, application_id, message, direction: 'outbound'
    }).select().single();
    if (error) throw error;
    res.status(201).json(data);
  } catch (error: any) {
...
router.put('/profile', authenticate, async (req: Request, res: Response) => {
  ...
  try {
    ...
    const { data, error } = await supabase.from('job_providers').update(updates).eq('user_id', userId).select().single();
    if (error) throw error;
    res.json(data);
  } catch (error: any) {
    res.status(500).json({ error: error.message });
...
router.patch('/settings', authenticate, async (req: Request, res: Response) => {
  ...
  try {
    const { data, error } = await supabase.from('job_providers').update({
      email_notifications: email_notifications ?? true,
      sms_notifications: sms_notifications ?? false,
      auto_respond: auto_respond ?? false,
    }).eq('user_id', userId).select().single();
    if (error) throw error;
    res.json({ success: true, settings: { email_notifications: data.email_notifications, sms_notifications: data.sms_notifications, auto_respond: data.auto_respond } });
  } catch (error: any) {
    res.status(500).json({ error: error.message });
  }
});
