...
export class StudentFeePaymentController {
  ...
  async getReceipt(req: Request, res: Response) {
    ...
    try {
      const { data, error } = await supabase.from('fee_payments').select('*, student_fee:student_fees(*), student:students(full_name, roll_number, student_class)').eq('id', payment_id).single();
