...
export class AnalyticsService {
  ...
  async updateOrganisation(id: string, body: any) {
    try {
      const { data } = await supabase.from('organisations').update(body).eq('id', id).select().single();
      return data;
    } catch {
      return null;
    }
  ...
  async bulkCreateOrganisations(orgs: Array<{ name: string; email: string; phone?: string; address?: string }>) {
