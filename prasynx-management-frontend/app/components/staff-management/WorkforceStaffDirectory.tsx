    ...
    <div className="w-full min-w-0">
      ...
      <Card className="p-4 mb-6">
        <div className="flex flex-wrap items-center gap-3">
          ...
          <button onClick={() => { const input = document.createElement('input'); input.type = 'file'; input.accept = '.csv,.xlsx'; input.click(); }} className="flex items-center gap-1.5 px-3.5 py-2 rounded-lg border border-gray-200 text-gray-600 text-xs font-semibold hover:bg-gray-50 transition-all"><Upload size={14} /> Import Staff</button>
          <button onClick={() => toast.success('Exporting staff data...')} className="flex items-center gap-1.5 px-3.5 py-2 rounded-lg border border-gray-200 text-gray-600 text-xs font-semibold hover:bg-gray-50 transition-all"><Download size={14} /> Export Staff</button>
          <button onClick={() => setShowAssignModal({ type: 'role' })} className="flex items-center gap-1.5 px-3.5 py-2 rounded-lg border border-gray-200 text-gray-600 text-xs font-semibold hover:bg-gray-50 transition-all"><Shield size={14} /> Assign Role</button>
          <button onClick={() => setShowAssignModal({ type: 'department' })} className="flex items-center gap-1.5 px-3.5 py-2 rounded-lg border border-gray-200 text-gray-600 text-xs font-semibold hover:bg-gray-50 transition-all"><Building2 size={14} /> Assign Department</button>
          <button onClick={() => toast.success('Opening work assignment...')} className="flex items-center gap-1.5 px-3.5 py-2 rounded-lg border border-gray-200 text-gray-600 text-xs font-semibold hover:bg-gray-50 transition-all"><Briefcase size={14} /> Assign Work</button>
        </div>
      </Card>
      ...
      <Card className="p-4 mb-6">
        <div className="flex flex-wrap items-center gap-3">
          ...
          <select value={deptFilter} onChange={e => setDeptFilter(e.target.value)} className="px-3 py-2 border border-gray-200 rounded-lg text-xs">
            <option value="">All Departments</option>
            {deptList.map((d: any) => <option key={d.id || d} value={d.name || d}>{d.name || d}</option>)}
          </select>
          <select value={roleFilter} onChange={e => setRoleFilter(e.target.value)} className="px-3 py-2 border border-gray-200 rounded-lg text-xs">
            ...
            <tbody>
              {staffList.length === 0 ? (
                ...
                <tr key={staff.id || i} className={`border-t border-gray-50 hover:bg-[#6D4CFF]/[0.02] transition-colors ${i % 2 === 0 ? 'bg-white' : 'bg-gray-50/30'}`}>
                  <td className="px-4 py-3 font-mono text-[10px] text-gray-400">{staff.staff_unique_id || staff.teacher_code || staff.id?.slice(0, 8) || '—'}</td>
                  <td className="px-4 py-3"><span className="font-semibold text-gray-900">{staff.full_name || staff.name || '—'}</span></td>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/30 backdrop-blur-sm" onClick={() => setShowAddModal(false)}>
          <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} className="bg-white rounded-2xl shadow-2xl w-full max-w-lg max-h-[85vh] overflow-y-auto" onClick={e => e.stopPropagation()}>
            <div className="flex items-center justify-between p-5 border-b border-gray-100">
              ...
              <button onClick={() => setShowAddModal(false)} className="w-7 h-7 rounded-lg hover:bg-gray-100 flex items-center justify-center text-gray-400 hover:text-gray-700 text-lg">×</button>
            </div>
      ...
      {showAssignModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/30 backdrop-blur-sm" onClick={() => setShowAssignModal(null)}>
          <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} className="bg-white rounded-2xl shadow-2xl w-full max-w-md max-h-[85vh] overflow-y-auto" onClick={e => e.stopPropagation()}>
            <div className="flex items-center justify-between p-5 border-b border-gray-100">
              <h3 className="font-bold text-sm">Assign {showAssignModal.type === 'role' ? 'Role' : 'Department'}</h3>
              <button onClick={() => setShowAssignModal(null)} className="w-7 h-7 rounded-lg hover:bg-gray-100 flex items-center justify-center text-gray-400 hover:text-gray-700 text-lg">×</button>
            </div>
            <div className="p-5 space-y-4">
