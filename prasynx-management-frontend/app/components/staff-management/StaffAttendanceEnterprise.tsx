...
const STATUS_BTN_STYLES: Record<string, string> = {
  ...
  'Half Day': 'bg-orange-100 text-orange-700 hover:bg-orange-200 border-orange-200',
  'Work From Home': 'bg-teal-100 text-teal-700 hover:bg-teal-200 border-teal-200',
};
...
export function StaffAttendanceEnterprise() {
  ...
  const TabButton = ({ tab }: { tab: Tab }) => (
    <button onClick={() => setActiveTab(tab)}
      className={`px-4 py-2.5 text-xs font-semibold rounded-lg transition-all whitespace-nowrap ${activeTab === tab ? 'bg-[#6D4CFF] text-white shadow-md' : 'text-gray-500 hover:bg-gray-100'}`}>
      {tab}
    </button>
  );
    ...
    <div className="w-full min-w-0">
      ...
      <div className="flex items-center gap-2 mb-6 overflow-x-auto pb-1">
        ...
        <div className="ml-auto flex items-center gap-2">
          <button onClick={() => analytics.refetch()} className="p-2 rounded-lg border border-gray-200 text-gray-400 hover:bg-gray-50 transition" title="Refresh"><RefreshCw size={14} /></button>
        </div>
      </div>
      ...
      {activeTab === 'Overview' && (
        <div className="space-y-6">
          <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-5 gap-4">
            <Card className="p-4"><div className="text-[10px] font-bold uppercase tracking-wider text-gray-400">Total Staff</div><div className="text-2xl font-extrabold text-gray-900 mt-2">{totalStaff}</div></Card>
            <Card className="p-4"><div className="text-[10px] font-bold uppercase tracking-wider text-green-500">Present Today</div><div className="text-2xl font-extrabold text-green-600 mt-2">{presentToday}</div></Card>
            <Card className="p-4"><div className="text-[10px] font-bold uppercase tracking-wider text-red-500">Absent Today</div><div className="text-2xl font-extrabold text-red-600 mt-2">{absentToday}</div></Card>
            <Card className="p-4"><div className="text-[10px] font-bold uppercase tracking-wider text-amber-500">Late Today</div><div className="text-2xl font-extrabold text-amber-600 mt-2">{lateToday}</div></Card>
            <Card className="p-4"><div className="text-[10px] font-bold uppercase tracking-wider text-teal-500">WFH Today</div><div className="text-2xl font-extrabold text-teal-600 mt-2">{wfhToday}</div></Card>
          </div>

          <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
            <Card className="p-4 lg:col-span-2">
              <h3 className="text-xs font-bold text-gray-700 mb-4 uppercase tracking-wider">Weekly Attendance Rate (%)</h3>
              <div className="h-64">
            ...
            <Card className="p-4">
              <h3 className="text-xs font-bold text-gray-700 mb-4 uppercase tracking-wider">Distribution</h3>
              <div className="h-64">
                <ResponsiveContainer width="100%" height="100%">
                  <PieChart>
          ...
          <Card className="p-4">
            <h3 className="text-xs font-bold text-gray-700 mb-4 uppercase tracking-wider">Department Breakdown</h3>
            <div className="h-72">
              <ResponsiveContainer width="100%" height="100%">
          ...
          <Card className="p-4">
            <h3 className="text-xs font-bold text-gray-700 mb-4 uppercase tracking-wider">Monthly Trend</h3>
            <div className="h-64">
              <ResponsiveContainer width="100%" height="100%">
      ...
      {activeTab === 'Mark Attendance' && (
        <div className="space-y-6">
          <Card className="p-4">
            <div className="flex flex-col sm:flex-row sm:items-center gap-4 mb-4">
              <div className="flex items-center gap-2 bg-gray-50 border border-gray-200 px-3 py-2 rounded-xl text-xs">
                ...
                <input type="date" value={selectedDate} onChange={e => setSelectedDate(e.target.value)} className="bg-transparent font-medium text-gray-700 focus:outline-none" />
              </div>
              <div className="relative flex-1">
                <Search size={14} className="absolute left-3 top-1/2 -translate-y-1/2 text-gray-400" />
                <input placeholder="Search staff..." className="w-full pl-9 pr-4 py-2 border border-gray-200 rounded-xl text-xs focus:outline-none focus:ring-2 focus:ring-[#6D4CFF]/20" />
              </div>
            ...
            <div className="overflow-x-auto">
              <table className="w-full text-xs">
                <thead><tr className="bg-gray-50 border-b border-gray-100">
                  <th className="text-left px-4 py-3 font-semibold text-gray-500">Staff</th>
          ...
          <Card className="p-4">
            <h3 className="text-xs font-bold text-gray-700 mb-3 uppercase tracking-wider">Quick Controls</h3>
            <div className="flex flex-wrap gap-2">
              <button onClick={() => toast.success('All marked Present')} className="px-4 py-2 rounded-lg bg-green-50 text-green-700 text-xs font-semibold border border-green-200 hover:bg-green-100 transition">Mark All Present</button>
              <button onClick={() => toast.success('All marked Absent')} className="px-4 py-2 rounded-lg bg-red-50 text-red-700 text-xs font-semibold border border-red-200 hover:bg-red-100 transition">Mark All Absent</button>
              ...
              <button onClick={() => toast.success('Attendance synced to payroll')} className="px-4 py-2 rounded-lg border border-gray-200 text-gray-600 text-xs font-semibold hover:bg-gray-50 transition">Sync to Payroll</button>
            </div>
          </Card>
        </div>
      )}
        ...
        <div className="space-y-4">
          <Card className="p-4">
            <div className="flex items-center justify-between mb-4">
              <h3 className="text-xs font-bold text-gray-700 uppercase tracking-wider">Pending Corrections</h3>
            ...
            {correctionsList.length === 0 ? (
              ...
              <div className="space-y-3">
                {correctionsList.map((c: any, i: number) => (
                  <div key={c.id || i} className="flex items-center justify-between p-3 bg-amber-50 rounded-xl border border-amber-100">
                    <div className="flex items-center gap-3">
                      <AlertTriangle size={16} className="text-amber-500" />
                      <div><p className="text-xs font-semibold text-gray-800">{c.staff_name || c.name || 'Staff'}</p><p className="text-[10px] text-gray-400">{c.reason || c.description || 'Missed punch-in correction request'}</p></div>
                    </div>
                    <div className="flex gap-2">
                      <button onClick={() => toast.success('Correction approved')} className="px-3 py-1.5 rounded-lg bg-green-50 text-green-700 text-[10px] font-semibold border border-green-200 hover:bg-green-100 transition"><CheckCircle2 size={12} className="inline mr-1" />Approve</button>
                      <button onClick={() => toast.success('Correction rejected')} className="px-3 py-1.5 rounded-lg bg-red-50 text-red-700 text-[10px] font-semibold border border-red-200 hover:bg-red-100 transition"><XCircle size={12} className="inline mr-1" />Reject</button>
                    </div>
                  </div>
                ))}
              </div>
      ...
      {activeTab === 'Heatmap' && (
        <div className="space-y-4">
          <Card className="p-4">
            <h3 className="text-xs font-bold text-gray-700 mb-4 uppercase tracking-wider">Attendance Heatmap</h3>
            ...
            {[
              ...
              <Card key={i} className="p-4 hover:shadow-md transition cursor-pointer" onClick={() => toast.success(`Generating ${r.label}...`)}>
                <div className="flex items-start gap-3">
                  <div className="w-9 h-9 rounded-lg bg-[#F0EDFF] flex items-center justify-center text-[#6D4CFF]"><r.icon size={18} /></div>
                  <div><h4 className="text-xs font-bold text-gray-800">{r.label}</h4><p className="text-[10px] text-gray-400 mt-0.5">{r.desc}</p></div>
                </div>
              </Card>
            ))}
          </div>
