        ...
        <div className="flex flex-wrap items-center justify-between gap-3">
          <div className="flex items-center gap-3">
            <div className="flex items-center gap-2 bg-gray-50 rounded-lg px-3 py-2">
              ...
              <input type="date" value={selectedDate} onChange={e => setSelectedDate(e.target.value)} className="bg-transparent text-xs font-medium focus:outline-none" />
            </div>
          ...
          <div className="flex items-center gap-2">
            <button onClick={() => handleBulkAttendance('Present')} className="px-3 py-1.5 rounded-lg bg-green-100 text-green-700 text-xs font-semibold hover:bg-green-200">Bulk Present</button>
            <button onClick={() => handleBulkAttendance('Absent')} className="px-3 py-1.5 rounded-lg bg-red-100 text-red-700 text-xs font-semibold hover:bg-red-200">Bulk Absent</button>
            <button onClick={() => toast.success('Report exported')} className="px-3 py-1.5 rounded-lg border border-gray-200 text-gray-600 text-xs font-semibold hover:bg-gray-50"><Download size={14} className="inline mr-1" />Export</button>
          </div>
        </div>
      </Card>
            ...
            <tbody>
              {staffList.length === 0 ? (
                ...
                <tr key={staff.id || i} className={`border-t border-gray-50 hover:bg-[#6D4CFF]/[0.02] transition-colors ${i % 2 === 0 ? 'bg-white' : 'bg-gray-50/30'}`}>
                  ...
                  <td className="px-4 py-3 text-gray-500">{staff.department || staff.department_name || '—'}</td>
                    ...
                    <div className="flex gap-1 flex-wrap">
                      {ATTENDANCE_STATUSES.map(status => (
                        <button key={status} onClick={() => handleMarkAttendance(staff.id, status)}
                          className={`px-2 py-1 rounded text-[9px] font-semibold border transition-all ${STATUS_BTN_STYLES[status] || 'bg-gray-100 text-gray-600'}`}>
