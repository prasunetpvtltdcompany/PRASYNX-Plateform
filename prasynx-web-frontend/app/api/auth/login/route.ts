...
const BACKENDS: Record<string, string> = {
  ...
  recruiter: process.env.NEXT_PUBLIC_BACKEND_RECRUITER || 'http://localhost:4006/api/login',
  admin: process.env.NEXT_PUBLIC_BACKEND_ADMIN || 'http://localhost:4001/api/admin/login',
};
...
export async function POST(req: NextRequest) {
  try {
    ...
    if (role === 'recruiter') {
      return NextResponse.json({ token: data.token, user: data.provider });
    }
    if (role === 'admin') {
