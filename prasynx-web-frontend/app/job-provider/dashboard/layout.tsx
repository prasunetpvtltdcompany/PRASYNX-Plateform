    ...
    <div className="flex min-h-screen bg-[#F8FAFF]">
      <aside className="fixed left-0 top-0 z-40 flex h-screen w-64 flex-col border-r border-[#E2E8F0] bg-white">
        ...
        <nav className="flex-1 space-y-1 p-4">
          {[
            { href: '/job-provider/dashboard', label: 'Dashboard' },
            { href: '/job-provider/jobs', label: 'My Jobs' },
            { href: '/job-provider/candidates', label: 'Candidates' },
            { href: '/job-provider/post', label: 'Post a Job' },
            { href: '/job-provider/profile', label: 'Profile' },
          ].map((item) => (
        ...
        <div className="border-t border-[#E2E8F0] p-4">
          <div className="flex items-center gap-3">
            ...
            <div className="min-w-0 flex-1">
              <p className="truncate text-xs font-bold text-slate-900">Job Provider</p>
              <p className="text-[10px] text-slate-500 capitalize">Recruiter</p>
            </div>
          </div>
        </div>
      </aside>
